<?php
	...
	$filedata = addslashes(fread(fopen($_FILES['file_buku']['tmp_name'], 'r'),
               $_FILES['file_buku']['size']));
	...
	if ($filetype=='application/pdf' and $cover2=='image/JPG' || $cover2=='image/png' || $cover2=='image/jpeg' || $cover2=='image/jpg' ) {
		if(move_uploaded_file($_FILES['file_buku']['tmp_name'],$loc_file) && move_uploaded_file($_FILES['cover_buku']['tmp_name'],$loc_cover))
		{
			$q ="INSERT INTO upload values('','$nama_buku','$penerbit','$pengarang','$tahun_terbit','$jns_buku','upload/cover/$cover','upload/file/$filename','$filetype','$filedata','$filename','$filesize','$nama_input')";
			mysqli_query($koneksi,$q);

			$upl= basename( $_FILES['file_buku']['name']);
			echo "<script>alert('File $upl Berhasil di Upload ');window.location.href='book-admin.php'</script>";
